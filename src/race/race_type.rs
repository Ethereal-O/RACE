...
impl Header {
    pub fn get_local_depth(&self) -> u8 {
        (self.data >> size_of::<u64>() - size_of::<u8>()) as u8
    }

    pub fn set_local_depth(&mut self, depth: u8) {
        self.data = (self.data
            & !(0xFF << size_of::<u64>() - size_of::<u8>()))
            | ((depth as u64) << size_of::<u64>() - size_of::<u8>());
    }
}
...
pub struct Subtable {
    ...
    pub bucket_groups: Box<[BucketGroup; CONFIG.bucket_group_num]>,
}

pub struct Directory {
    pub suffix: String,
    pub subtable: Box<Subtable>,
}
